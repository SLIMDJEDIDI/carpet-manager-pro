...
export default async function JaxManagementPage({
  ...
  const dispatches = await prisma.jaxLog.findMany({
    where: {
      status: activeTab === "pending" ? "SUCCESS" : "DISPATCHED",
      OR: [
        { trackingId: { contains: query, mode: "insensitive" } },
        { order: { customerName: { contains: query, mode: "insensitive" } } },
        { order: { customerPhone: { contains: query, mode: "insensitive" } } },
      ],
    },
    include: {
      order: true,
    ...
    <div className="space-y-6 md:space-y-10">
      ...
      <div className={`${activeTab === "pending" ? "bg-indigo-900 shadow-indigo-200/50" : "bg-emerald-900 shadow-emerald-200/50"} rounded-[2rem] p-6 text-white flex flex-wrap items-center gap-8 shadow-xl relative overflow-hidden transition-colors duration-500`}>
        ...
        <div className="relative z-10 flex items-center gap-4 border-r border-white/20 pr-8">
          ...
          <div>
            ...
            <p className="text-2xl font-black leading-none">{dispatches.length} Parcels</p>
          </div>
        </div>
        <div className="relative z-10 hidden md:block">
          <p className="text-white/60 text-xs font-bold max-w-xs leading-relaxed">
            {activeTab === "pending" 
              ? "These parcels have been registered in JAX. Print the labels and stick them to the parcels."
              : "Historical list of parcels handed over to JAX. Use these for customer support or tracking."}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        {dispatches.map((dispatch) => {
          const { order, items } = dispatch;
          const codAmount = items.reduce((sum, i) => sum + i.price, 0);

          return (
            <div key={dispatch.id} className="bg-white rounded-[2rem] border border-slate-100 shadow-sm hover:shadow-2xl transition-all duration-500 group overflow-hidden">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-sm font-bold text-slate-500">
                        {activeTab === "pending" ? "Registered" : "Dispatched"}: {new Date(dispatch.createdAt).toLocaleString('fr-TN')}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-slate-200/60">
                    <div className={`rounded-2xl p-4 border-2 border-dashed ${activeTab === "pending" ? "bg-white border-indigo-200" : "bg-emerald-50 border-emerald-200"}`}>
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <span className={`text-lg font-black tracking-tight ${activeTab === "pending" ? "text-indigo-700" : "text-emerald-700"} truncate`}>{dispatch.trackingId}</span>
                        <div className="flex items-center gap-1">
                          <CopyButton text={dispatch.trackingId || ""} />
                          <a 
